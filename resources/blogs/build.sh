echo "==== Building Blogs ===="
buildDir="../../WMaxZimmerman.Site.DAL/resources/blogs"
mkdir -p $buildDir
...
do
    name=$(basename $f | cut -f 1 -d '.')
    echo "    <None Remove=\"resources\\blogs\\$name.html\" />" >> $buildDir
done
echo "  </ItemGroup>" >> $buildDir
...
do
    name=$(basename $f | cut -f 1 -d '.')
    echo "    <EmbeddedResource Include=\"resources\\blogs\\$name.html\">" >> $buildDir
